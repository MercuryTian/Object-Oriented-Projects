...
//		}
		...
		//System.out.println(d + " " + location.getX() + " " + location.getY());
	...
	public int DirExchange(Point pre, Point togo){
		//这个方法在第二次打车程序中好像木有什么用了> <
		/*
		 * Used to judge the relationship between two points on map.
		 * REQUIRES: Must provide two variables in Point type.
		 * 			 The former is the present point, the later is the target point.
		 * MODIFIES: None.
		 * EFFECTS: If the present point is under the target point, the moving direction is upward.
		 * 			Then return 0.
		 * 			If the present point is above the target point, the moving direction is downward.
		 * 			Then return 1.
		 *  		If the present point is on the right side of the target point, the moving direction is left.
		 * 			Then return 2.
		 *  		If the present point is on the left side of the target point, the moving direction is right.
		 * 			Then return 3.	
		 */
		/*
		 * direction值:
