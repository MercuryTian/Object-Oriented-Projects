...
public class Map {
	...
	private String crossPath = "Cross.txt";	//道路交叉情况文件的路径
	...
	private static int[][] cross;	//0：立体交叉；1：平面交叉
	private static TrafficLight[][] light;	//红绿灯情况
	
	
	public static String getLight(Point p, int dir) {
		//dir:表示南北还是东西：0：南北，1：东西
		if(light[p.getX()][p.getY()].getLight_NS() == 2 && dir == 0)
			return "NS";
		else if(light[p.getX()][p.getY()].getLight_NS() == 1 && dir == 0)
			return "NSred";
		else if(light[p.getX()][p.getY()].getLight_WE() == 2 && dir == 1)
			return "WE";
		else if(light[p.getX()][p.getY()].getLight_WE() == 1 && dir == 1)
			return "WEred";
		else
			return "NO";
	}
...
//				}	
		...
		try {
			...
			BufferedReader buffer = new BufferedReader(crossfile);
			String s = null;
			i = 0;
			while((s=buffer.readLine()) != null){
				...
				for(int j=0; j<s.length(); j++){
					int cross_value = s.charAt(j) -'0';
					cross[i][j] = cross_value;
				}
				i++;
			}
			buffer.close();
		...
		for(int k=0; k<80; k++){
			for(int j=0; j<80; j++){
				count = CountPoints(matrix[k][j]);
				if(cross[k][j]==0 || count<3){
					//立体交叉和相邻点数小于3不设红绿灯
					light[k][j] = new TrafficLight(false);
				}
				else{
					light[k][j] = new TrafficLight(true);
//					light[k][j].start();
				}
			}
		}
	...
	public int CountPoints(Point p){
		if(p.getConnect(0) && p.getConnect(1) && p.getConnect(2) && p.getConnect(3)){
			return 4;
		}
		else if((p.getConnect(0)&&!p.getConnect(1)&&!p.getConnect(2)&&!p.getConnect(3)) || 
				(!p.getConnect(0)&&p.getConnect(1)&&!p.getConnect(2)&&!p.getConnect(3)) || 
				(!p.getConnect(0)&&!p.getConnect(1)&&p.getConnect(2)&&!p.getConnect(3)) || 
				(!p.getConnect(0)&&!p.getConnect(1)&&!p.getConnect(2)&&p.getConnect(3))){
			return 1;
		}
...
//			System.out.println("Wrong direction");
	...
	public boolean repOK(){
		if(PATH == null || crossPath == null)
			return false;
		if(map_matrix == null || matrix == null || changeInfo == null || trafficFlow == null ||
				cross == null || light == null)	
			return false;
		return true;
	}
